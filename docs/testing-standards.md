...
- **No filesystem dependence** — `pyfakefs` (`fs` fixture) for tests that read/write files; never repo-relative paths. Exception: subprocess-based tests (the CLI shell-out in `test_cli.py`, driver runs in `test_finish_translations.py`) keep `tmp_path` — the child process sees the real filesystem.
